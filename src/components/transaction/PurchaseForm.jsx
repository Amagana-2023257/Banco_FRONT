...
export const PurchaseForm = () => {
  const { purchase, txn, balance, isLoading: isPurchasing, error: purchaseError } = usePurchase();
  ...
  const handleSubmit = e => {
    e.preventDefault();
    purchase({ accountId: form.accountId, amount: Number(form.amount) });
  };
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <button type="submit" className="btn btn-primary w-100" disabled={isPurchasing || loadingAccounts}>
        {isPurchasing ? 'Procesando...' : 'Comprar'}
      </button>

      {purchaseError && <div className="alert alert-danger mt-3">{purchaseError}</div>}
      {txn && (
        <div className="alert alert-success mt-3">
          Compra OK. Nuevo saldo: Q{balance.toFixed(2)}
        </div>
      )}
    </form>
  );
